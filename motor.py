...
def preguntar(key, estado, preguntas, modelo="jev-latest", intentos=5):
    ...
    request = urllib.request.Request(
        URL, data=body, headers={"Authorization": f"Bearer {key}", "Content-Type": "application/json"}
    )
    for intento in range(intentos):
        ...
        try:
            ...
            if error.code in (429, 503, 529) and intento < intentos - 1:
                time.sleep(0.6 * 2**intento)
                continue
            raise RuntimeError(f"Jev respondió HTTP {error.code}: {detalle}") from None
        except (urllib.error.URLError, TimeoutError) as error:
            permanente = isinstance(getattr(error, "reason", None), ssl.SSLCertVerificationError)
            if intento < intentos - 1 and not permanente:
                time.sleep(0.6 * 2**intento)
                continue
...
def torneo(animales, n, arena="tierra", modo="turbo", azar=False, semilla=None, tipos=None,
           modelo="jev-latest", cancelar=None, guardar=True):
